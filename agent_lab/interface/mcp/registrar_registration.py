...
class RegistrarDescriptor:
    ...
    # @discoverable_agent for agent extra dependencies.


_registry: dict = {}  # keyed by class, preserving decoration order


def discoverable_mcp_registrar(*, extra_deps: Sequence[str] = ()):
    ...
    kwarg. Mirrors ``@discoverable_agent`` so downstream apps contribute registrars
    ...
    def decorator(cls):
        ...
        if not (isinstance(cls, type) and issubclass(cls, McpRegistrar)):
            raise TypeError(
                f"@discoverable_mcp_registrar requires an McpRegistrar subclass, got {cls!r}"
            )
