...
def scan_packages(package_names: Sequence[str]) -> None:
    """Imports every module under each package so @RegisterAgent decorators fire.

    Accepts plain modules too (imported as-is). Re-imports are no-ops via
    ``sys.modules``, so scanning is idempotent across repeated create_app calls.
    """
...
def load_entry_point_agents(group: str = ENTRY_POINT_GROUP) -> None:
    ...
    loading the entry point imports its target, firing @RegisterAgent.
    """
